...
const CW = 780; // content width (900 - 2*60)
...
function TitleScene({ data, rng }) {
  ...
  return (
    <>
      <AnimatedText x={450} y={50} fontSize={42} fontWeight={600} textAnchor="middle" delay={0.2}>
        {title}
      </AnimatedText>
      <AnimatedPath
        d={roughUnderline(450 - titleWidth / 2, 62, titleWidth, rng, 3)}
        delay={0.7}
        duration={0.8}
        strokeWidth={3}
      />
      {subtitle && (
        <AnimatedText x={450} y={95} fontSize={22} fill="#52525B" textAnchor="middle" delay={1.0}>
          {subtitle}
        </AnimatedText>
      )}
    </>
...
function ProblemFrameScene({ data, rng }) {
  ...
  return (
    <>
      <AnimatedPath
        d={roughRect(MX, 10, CW, 130, rng, 2)}
        delay={0.1}
        duration={1.2}
        strokeWidth={2}
        stroke="#0055FF"
      />
      <AnimatedText x={80} y={35} fontSize={14} fill="#0055FF" fontFamily="Outfit, sans-serif" fontWeight={600} delay={0.3}>
        PROBLEM
      </AnimatedText>
      <AnimatedText x={80} y={72} fontSize={28} fontWeight={600} delay={0.6} maxWidth={38} lineHeight={34}>
        {question}
      </AnimatedText>
      {context && (
        <AnimatedText x={80} y={115} fontSize={18} fill="#52525B" delay={1.0} maxWidth={50}>
          {context}
        </AnimatedText>
      )}
    </>
...
function ComparisonScene({ data, rng }) {
  ...
  return (
    <>
      ...
      <AnimatedPath
        d={roughLine(MX, 75, MX + CW, 75, rng, 1)}
        delay={0.4}
        duration={0.6}
        stroke="#D4D4D8"
      />
    ...
    <>
      ...
      {pros.map((pro, i) => (
        <AnimatedGroup key={`pro-${i}`} delay={0.6 + i * 0.15}>
          <AnimatedPath
            d={roughCheckmark(MX + 16, 118 + i * 34, 14)}
            delay={0.65 + i * 0.15}
            duration={0.3}
            stroke="#22C55E"
            strokeWidth={2.5}
      ...
      {cons.map((con, i) => (
        <AnimatedGroup key={`con-${i}`} delay={0.6 + i * 0.15}>
          <AnimatedPath
            d={roughCross(MX + halfW + 16, 118 + i * 34, 14)}
            delay={0.65 + i * 0.15}
            duration={0.3}
            stroke="#E63946"
            strokeWidth={2.5}
...
function ChecklistScene({ data, rng }) {
  ...
  return (
    <>
      ...
      <AnimatedPath
        d={roughLine(MX, 75, MX + 300, 75, rng, 1)}
        delay={0.3}
        duration={0.4}
        stroke="#D4D4D8"
      />
      ...
      {items.map((item, i) => {
        ...
        return (
          <AnimatedGroup key={i} delay={0.4 + i * 0.2}>
            {/* Checkbox */}
            <AnimatedPath
              d={roughRect(MX + 10, iy, 20, 20, rng, 1)}
              delay={0.45 + i * 0.2}
              duration={0.3}
              strokeWidth={2}
              stroke={isChecked ? "#22C55E" : "#D4D4D8"}
            />
            {isChecked && (
              <AnimatedPath
                d={roughCheckmark(MX + 12, iy + 2, 16)}
                delay={0.55 + i * 0.2}
                duration={0.3}
                stroke="#22C55E"
                strokeWidth={2.5}
...
function ScorecardScene({ data, rng }) {
  ...
  return (
    <>
      ...
      {/* Header row */}
      <AnimatedPath
        d={roughLine(MX, 80, MX + CW, 80, rng, 1)}
        delay={0.3}
        duration={0.5}
        strokeWidth={2}
      />
      ...
      <AnimatedPath
        d={roughLine(MX, 110, MX + CW, 110, rng, 1)}
        delay={0.5}
        duration={0.4}
        stroke="#D4D4D8"
      />
        ...
        return (
          <AnimatedGroup key={ci} delay={0.6 + ci * 0.15}>
            ...
            {options.map((opt, oi) => {
              ...
              const isMax =
                options.every((o) => ((o.scores || [])[ci] || 0) <= score) && score > 0;
              return (
                <AnimatedText
                  key={`s-${oi}`}
                  x={MX + colW * (oi + 1) + colW / 2}
                  ...
                  fill={isMax ? "#0055FF" : "#3F3F46"}
                  textAnchor="middle"
                  delay={0.65 + ci * 0.15 + oi * 0.05}
                >
                  {score}
                </AnimatedText>
              );
            ...
            {ci < criteria.length - 1 && (
              <AnimatedPath
                d={roughLine(MX, ry + 32, MX + CW, ry + 32, rng, 0.5)}
                delay={0.7 + ci * 0.15}
                duration={0.3}
                stroke="#F4F4F5"
                strokeWidth={1}
      ...
      {(() => {
        ...
        return (
          <AnimatedGroup delay={0.7 + criteria.length * 0.15}>
            <AnimatedPath
              d={roughLine(MX, totalsY, MX + CW, totalsY, rng, 1)}
              delay={0.7 + criteria.length * 0.15}
              duration={0.4}
              strokeWidth={2}
            />
            <AnimatedText
              ...
              fontSize={18}
              fontWeight={700}
              delay={0.8 + criteria.length * 0.15}
            >
              Total
            </AnimatedText>
            {options.map((opt, oi) => {
              ...
              const maxTotal = Math.max(
                ...options.map((o) => (o.scores || []).reduce((a, b) => a + b, 0))
              );
              return (
                <AnimatedText
                  key={`t-${oi}`}
                  ...
                  fontSize={22}
                  fontWeight={700}
                  fill={total === maxTotal ? "#0055FF" : "#3F3F46"}
                  textAnchor="middle"
                  delay={0.85 + criteria.length * 0.15 + oi * 0.05}
                >
                  {total}
                </AnimatedText>
              );
...
function RecommendationScene({ data, rng }) {
  ...
  return (
    <>
      {/* Outer box */}
      <AnimatedPath
        d={roughRect(MX, 10, CW, 80 + reasons.length * 32 + 60, rng, 2)}
        delay={0.1}
        duration={1.2}
        strokeWidth={2.5}
        stroke="#0055FF"
      ...
      <AnimatedText x={80} y={92} fontSize={26} fontWeight={600} delay={0.6} maxWidth={38} lineHeight={32}>
        {recommendation}
      </AnimatedText>
...
function NotesScene({ data, rng }) {
  ...
  return (
    <>
      ...
      <AnimatedPath
        d={roughLine(MX, 75, MX + 240, 75, rng, 1)}
        delay={0.3}
        duration={0.4}
        stroke="#D4D4D8"
      />
      ...
      {notes.map((note, i) => {
        ...
        return (
          <AnimatedGroup key={i} delay={0.4 + i * 0.15}>
            ...
            <AnimatedText x={MX + 32} y={ny + 14} fontSize={18} fill="#3F3F46" delay={0.45 + i * 0.15} maxWidth={42}>
              {note}
            </AnimatedText>
          </AnimatedGroup>
        );
